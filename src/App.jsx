...
import { Copy, Check, RotateCcw, Save, Clock, X, Moon, Sun, ChevronDown, ChevronUp, FlaskConical, FileText, ClipboardList } from "lucide-react";
...
const TEMPLATES = [
  {
    label: "LP Done",
    icon: "🩸",
    text: "Lumbar puncture was attempted under aseptic technique after informed consent. Procedure was [successful / unsuccessful] with [CSF obtained / no CSF obtained]. Patient tolerated the procedure [well / with difficulty] and vitals remained stable."
  },
  {
    label: "LP Refused",
    icon: "🚫",
    text: "LP was explained to the mother including indication, procedure, and possible risks/benefits.\nThe mother refused the procedure and signed refusal form."
  },
  {
    label: "DAMA",
    icon: "⚠️",
    text: "The mother requested discharge Against Medical Advice (DAMA). The patient's condition, management plan, and potential risks and complications were explained thoroughly, and all her questions were answered. Despite this, she insisted on proceeding with DAMA and signed the DAMA form."
  },
];
...
const LabInput = ({ k, label, placeholder, labs, onChange, dark }) => (
  <div className="flex flex-col gap-1">
    ...
    <input
      ...
      placeholder={placeholder || ""} value={labs[k]}
    ...
    <div className="space-y-4 pb-10">
      ...
      <div className={`rounded-xl border shadow-sm overflow-hidden ${D ? "bg-gray-800 border-gray-700" : "bg-white border-gray-200"}`}>
        ...
        <div className="p-4 space-y-3">
          {BARCODES.map((item, i) => (
            <div key={i} className={`rounded-lg border p-3 ${D ? "border-gray-700 bg-gray-750" : "border-gray-100 bg-gray-50"}`}>
              <div className="flex items-center justify-between mb-2">
                <span className={`text-xs font-bold uppercase tracking-wide ${D ? "text-blue-400" : "text-blue-600"}`}>{item.label}</span>
                <button
                  onClick={() => copyText(item.codes, `barcode-${i}`)}
                  className={`flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg font-semibold transition-all ${copiedKey === `barcode-${i}` ? "bg-green-500 text-white" : "bg-blue-600 hover:bg-blue-700 text-white"}`}>
                  {copiedKey === `barcode-${i}` ? <><Check className="w-3 h-3" /> Copied!</> : <><Copy className="w-3 h-3" /> Copy</>}
                </button>
              </div>
          ...
          {TEMPLATES.map((item, i) => (
            <div key={i} className={`rounded-lg border p-3 ${D ? "border-gray-700 bg-gray-750" : "border-gray-100 bg-gray-50"}`}>
              <div className="flex items-center justify-between mb-2">
                ...
                <button
                  onClick={() => copyText(item.text, `template-${i}`)}
                  className={`flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg font-semibold transition-all ${copiedKey === `template-${i}` ? "bg-green-500 text-white" : "bg-purple-600 hover:bg-purple-700 text-white"}`}>
                  {copiedKey === `template-${i}` ? <><Check className="w-3 h-3" /> Copied!</> : <><Copy className="w-3 h-3" /> Copy</>}
                </button>
              </div>
...
export default function App() {
  const [page, setPage]           = useState("notes");   // "notes" | "references"
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${D ? "bg-gray-900" : "bg-gray-100"}`}>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className={`flex rounded-lg p-0.5 ${D ? "bg-gray-700" : "bg-gray-100"}`}>
            <button onClick={() => setPage("notes")}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-semibold transition-all ${page === "notes" ? "bg-blue-600 text-white shadow" : D ? "text-gray-400 hover:text-gray-200" : "text-gray-500 hover:text-gray-700"}`}>
              <ClipboardList className="w-3.5 h-3.5" /> Notes
            </button>
            <button onClick={() => setPage("references")}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-semibold transition-all ${page === "references" ? "bg-blue-600 text-white shadow" : D ? "text-gray-400 hover:text-gray-200" : "text-gray-500 hover:text-gray-700"}`}>
              <FlaskConical className="w-3.5 h-3.5" /> References
            </button>
          </div>
        </div>
              ...
              <Section title="Patient Info" icon="🧑‍⚕️" dark={D}>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input className={inp} placeholder="BABY OF SARA" value={s.name} onChange={e => setField("name", up(e.target.value))} />
                  </div>
                  <div>
                    <label className={lbl}>Age</label>
                    <input className={inp} placeholder="3 DAYS" value={s.age} onChange={e => setField("age", up(e.target.value))} />
                  </div>
                  {tab === "isbar" && <>
                    <div>
                      <label className={lbl}>MRN #</label>
                      <input className={inp} placeholder="123456" value={s.mrn} onChange={e => setField("mrn", up(e.target.value))} />
                    </div>
                    <div>
                      <label className={lbl}>Room #</label>
                      <input className={inp} placeholder="204" value={s.room} onChange={e => setField("room", up(e.target.value))} />
                    </div>
                  </>}
                </div>
                {tab === "progress" && (
                ...
                <div className="mt-3">
                  ...
                  <input className={inp} placeholder="NEONATAL JAUNDICE" value={s.dx} onChange={e => setField("dx", up(e.target.value))} />
                </div>
              </Section>
                ...
                <div className="space-y-3">
                  {[["general", "General"], ["chest", "Chest"], ["abd", "ABD"], ["cvs", "CVS"]].map(([key, label]) => (
                    <div key={key}>
                      ...
                      <input className={inpMono} value={s[key]} onChange={e => setField(key, up(e.target.value))} />
                    </div>
                  ))}
                </div>
              </Section>
      ...
      {/* ── Mobile Bottom Bar (Notes page only) ── */}
      {page === "notes" && (
